                ...
                <section className="grid grid-cols-4 gap-8 py-10">
                    <div className="row-span-2 col-span-3">
                        <div className="overflow-hidden rounded-lg">
                            {
                                data.front_image ?
                                <Image width={800} height={350} src={data.front_image} alt={data.name} className="object-cover max-w-[unset]" />
                                : ''
                            }
                        </div>
                        <h1 className="text-2xl font-bold mt-8 mb-4">{data.kost.name} - {data.name}</h1>
                        <label className="border rounded-sm px-4 border-bodydark1 mb-3 inline-block">{data.kost.category}</label>
                    ...
                    <div className="gap-8 flex flex-col">
                        <div className="overflow-hidden rounded-md">
                            { data.inside_image[0] ? 
                                <Image width={375} height={120} src={data.inside_image[0]} alt={data.name} className="object-cover max-w-[unset]" />
                            : ''}
                        </div>
                        <div className="overflow-hidden rounded-md">
                            { data.inside_image[1] ? 
                                <Image width={375} height={150} src={data.inside_image[1]} alt={data.name} className="object-cover max-w-[unset]" />
                            : ''}
                        </div>
                        <div className="text-center ">
                            <Link href="/" className="bg-[#25d366] px-4 py-2 rounded-sm text-white text-sm w-full flex justify-between items-center">
                                <Image width={20} height={20} src="/img/wa-white.png" alt="WA" />
                                <span>Hubungi {data.kost.user.mobile}</span>
                            </Link>
                        </div>
                    </div>
                </section>
